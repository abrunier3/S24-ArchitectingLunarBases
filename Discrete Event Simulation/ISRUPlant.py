...
class ISRUPlant:
    def __init__(self, system, processingRate, regHeadGrade):
        ...
        self.LOXStored = 0
        self.lg = 1.62 #Acceleration due to gravity on the moon, m/(s^2)
    ...
    def processRegolith(self, system, regolithMass, transportDist=1):
        ...
        E_exc = (6.5*10**-6)*regolithMass #Excavation Energy, kWh, Eq 5 in [1]
        E_trans = (3.6*10**-4)*regolithMass*transportDist #Transportation Energy, kWh, Eq 8 in [1]

        E_bene = ((regolithMass*self.lg*1)/3600000) + m_gangue*(3.6*10**-4)*1 #Beneficiation Energy, kWh, Eq 11 in [1], note that the first cosntant of 1 represents the height that the regolith needs to be lifted for beneficiation in meters, the second constant of 1 represents the distance that the waste material (gangue) is transported in km
        E_reactor =  0.385 * m_bene_reg #Energy Required By Reactor, kWh, Eq 14 in [1] (took the mean value of 0.385)
        E_electro = 8.26*m_H2O #Energy for Electrolysis, kWh, Eq 18 in [1]
        E_liq = 1.17*m_O2 #Liquefaction, kWh, Eq. 21 in [1]
